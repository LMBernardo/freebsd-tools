...
{
        dev=$1
        speed=`cat /var/run/dmesg.boot | grep "^${dev}:" | grep transfers | tail -1 | awk '{print $2};'`
        echo $speed
}
...
{
        dev=$1
        temp=`smartctl -A /dev/${dev} | grep Temperature_Celsius | awk '{print $10}'`
        echo $temp
}
...
echo -e "Temp\tDev\tSpeed\t\tName (Size)"
...
do
        ...
        if [ "$bus" == "ciss*" ]; then
                ...
                temp=`get_gen_disk_temp ${dev}`
        fi

        temp=`colorize_temperature ${temp}`

        echo -e "$temp\t${dev}\t${speed}\t${name} (${size}G)"
done
